...
// ValidateTrack checks if a track exists in catalog-service.
func (c *CatalogClient) ValidateTrack(ctx context.Context, trackID string) error {
	...
	if resp.StatusCode >= 300 {
		return fmt.Errorf("catalog responded with %d", resp.StatusCode)
	}
	// shallow decode to ensure payload is present
